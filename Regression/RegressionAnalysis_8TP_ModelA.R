...
#Filter out "bad" populations (1,2,4,5 and 6) and downsample to 8 timepoints
...
fiterms <- function(xx,format){
  ...
  p.week <- anova(time.reg)$"Pr(>F)"[1]
  
  data.frame("p.week"=p.week)
}
...
write.table(p_table,"Pval_table_8tp_ModelA.txt", quote= FALSE, row.names= FALSE,col.names= TRUE, sep="\t")
