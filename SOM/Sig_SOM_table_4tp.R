...
sig_snps <- subset(Full, p.week >= 2.5)
...
Sig_Traj_table_uni_dir_ds4 <- data.frame(matrix(ncol = 1, nrow = 4))
...
for (i in 1:nrow(maf)){
  ...
  Sig_Traj_table_uni_dir_ds4 <- cbind(Sig_Traj_table_uni_dir_ds4, maf_final)
  
  print(i)
}

Sig_Traj_table_uni_dir_ds4 <- Sig_Traj_table_uni_dir_ds4[,2:ncol(Sig_Traj_table_uni_dir_ds4)]



Trans_traj_sig_uni_dir_ds4 <- t(Sig_Traj_table_uni_dir_ds4)
save(Trans_traj_sig_uni_dir_ds4 , file="SOM_table_sig_4tp.rda")
