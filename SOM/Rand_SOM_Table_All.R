...
#list of random
Full <- read.table("~/Dropbox/Yeast_Time_Series/Revisions_1/Regression/Pval_table_17tp_All_Models.txt", header = TRUE)
Full$p.week <- -log10(Full$p.week)
sig_snps <-  Full[sample(1:nrow(Full), size = 958),]
...
Rand_Traj_table_uni_dir <- data.frame(matrix(ncol = 1, nrow = 17))
...
for (i in 1:nrow(maf)){
  ...
  Rand_Traj_table_uni_dir <- cbind(Rand_Traj_table_uni_dir, maf_final)
  
  print(i)
}

Rand_Traj_table_uni_dir <- Rand_Traj_table_uni_dir[,2:ncol(Rand_Traj_table_uni_dir)]


Trans_traj_rand_uni_dir <- t(Rand_Traj_table_uni_dir)
save(Trans_traj_rand_uni_dir , file="SOM_table_ramdom_all.rda")

load("/Users/markphillips/Dropbox/Yeast_Time_Series/Trans_Traj_rand_table_uni_dir.rda")
