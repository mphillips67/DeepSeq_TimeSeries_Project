...
#list of sigs
Full <- read.table("~/Dropbox/Yeast_Time_Series/Revisions_1/Regression/Pval_table_8tp_ModelA.txt", header = TRUE)
Full$p.week <- -log10(Full$p.week)
sig_snps <-  Full[sample(1:nrow(Full), size = 937),]
...
X <- rep(c(3,7:12),each =7)
Y <- c(2,6,8,10,12,14,18)
maf <- paste("maf_", X, "_", Y, sep="")
cov <- paste("cov_", X, "_", Y, sep="")
...
Rand_Traj_table_uni_dir_ds8 <- data.frame(matrix(ncol = 1, nrow = 8))
...
for (i in 1:nrow(maf)){
  ...
  matrix_maf <- data.frame(matrix(maf_snp, nrow = 7, ncol = 7))
  ancestor_maf <- rep(snps2018_v5$ancmaf_0_0[i],7)
  maf_final <- rbind(ancestor_maf, matrix_maf)
  colnames(maf_final) <- col_names
  Rand_Traj_table_uni_dir_ds8 <- cbind(Rand_Traj_table_uni_dir_ds8, maf_final)
  
  print(i)
}

Rand_Traj_table_uni_dir_ds8 <- Rand_Traj_table_uni_dir_ds8[,2:ncol(Rand_Traj_table_uni_dir_ds8)]



Trans_traj_rand_uni_dir_ds8 <- t(Rand_Traj_table_uni_dir_ds8)
save(Trans_traj_rand_uni_dir_ds8 , file="SOM_table_random_8tp.rda")
